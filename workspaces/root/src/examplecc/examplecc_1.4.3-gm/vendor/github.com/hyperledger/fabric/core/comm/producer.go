...
// Returns the connection, the endpoint selected, nil on success.
// Returns nil, "", error on failure
// By default, it will connect to an orderer configured by `peer.gossip.defaultOrdererAddress`
func (cp *ConnProducer) NewConnection() (*grpc.ClientConn, string, error) {
	defOrderer := GetDefaultOrdererAddress()
	logger.Debugf("Default Orderer Address is [%s]", defOrderer)
	if defOrderer != "" {
		curEndpoint := cp.getEndpointByAddress(defOrderer)
		if curEndpoint.Endpoint != defOrderer {
			logger.Error("Not found match endpoint by the Address")
		}
		conn, err := cp.connect(curEndpoint)
		if err != nil {
			logger.Error("Failed connecting to", defOrderer, ", error:", err)
		} else {
			return conn, defOrderer, nil
		}
	}
	cp.Lock()
	defer cp.Unlock()
...
func (cp *ConnProducer) getEndpointByAddress(addr string) EndpointCriteria {
	cp.Lock()
	defer cp.Unlock()
	for _, v := range cp.endpoints {
		if v.Endpoint == addr {
			return v
		}
	}
	return EndpointCriteria{}
}
