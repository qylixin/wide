...
func (cp *connProducer) NewConnection() (*grpc.ClientConn, string, error) {
	...
	if defOrderer != "" && cp.isInEndpoints(defOrderer) {
		conn, err := cp.connect(defOrderer)
		if err != nil {
...
// isInEndpoints returns true if addr value is in endpoints
func (cp *connProducer) isInEndpoints(addr string) bool {
	cp.Lock()
	defer cp.Unlock()
