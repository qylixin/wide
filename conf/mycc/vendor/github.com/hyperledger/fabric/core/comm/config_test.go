...
func TestConfig(t *testing.T) {
	...
	// reset cache
	configurationCached = false
	viper.Set("peer.gossip.defaultOrdererAddress", "orderer.example.com:7050")
	assert.EqualValues(t, "orderer.example.com:7050", GetDefaultOrdererAddress())
	viper.Set("peer.gossip.defaultOrdererAddress", "orderer02.example.com:7050")
	assert.NotEqual(t, "orderer02.example.com:7050", GetDefaultOrdererAddress())
	configurationCached = false
	viper.Set("peer.gossip.defaultOrdererAddress", "")
	assert.EqualValues(t, "", GetDefaultOrdererAddress())
}
